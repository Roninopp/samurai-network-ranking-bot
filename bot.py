...
async def start_battle_rps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data[2] < 10:  # Check if user has enough coins
...
async def handle_rps_battle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('bet_rps_'):
        # Set bet amount and start RPS game
        ...
        if len(parts) >= 5:
            ...
            if result == "win":
                ...
                final_reward = reward - bet_amount  # Net gain
            elif result == "lose":
                reward = 0
                result_text = "😞 *You lose...*"
                final_reward = -bet_amount  # Net loss
            else:
                reward = bet_amount  # Return bet
...
async def handle_dice_battle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('bet_dice_'):
        ...
        if player_roll > opponent_roll:
            ...
            result = "🤝 *It's a tie!*"
            await update_coins(user.id, bet_amount)  # Return bet
            net_gain = 0
...
async def handle_button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    handlers = {
        'main': lambda: query.edit_message_text(
        ...
        'pvp_duel': pvp_duel,
        'battle_rps': start_battle_rps,
        'battle_dice': start_battle_dice,
        'battle_stats': start_battle_stats,
        'team_battle': lambda: query.edit_message_text(
            "👥 *Team Battle*\n\nTeam features coming soon!", 
            reply_markup=back_button("battle_mode"),
            parse_mode='Markdown'
        ),
        'tournament': lambda: query.edit_message_text(
            "🏆 *Tournament*\n\nTournament mode launching soon!", 
            reply_markup=back_button("battle_mode"),
            parse_mode='Markdown'
        ),
        'battle_history': lambda: query.edit_message_text(
            "📜 *Battle History*\n\nYour battle records will appear here!", 
            reply_markup=back_button("battle_mode"),
            parse_mode='Markdown'
        ),
        'shop': lambda: query.edit_message_text(
            "🛍️ *Shop*\n\nAwesome items coming soon!", 
            reply_markup=back_button(),
            parse_mode='Markdown'
        ),
        'casino': lambda: query.edit_message_text(
            "🎰 *Casino*\n\nTry your luck at various games!", 
            reply_markup=back_button(),
            parse_mode='Markdown'
        ),
        'missions': lambda: query.edit_message_text(
            "🎯 *Daily Missions*\n\nComplete missions to earn rewards!", 
            reply_markup=back_button(),
            parse_mode='Markdown'
        ),
        'achievements': lambda: query.edit_message_text(
            "🎖️ *Achievements*\n\nUnlock achievements for special rewards!", 
            reply_markup=back_button(),
            parse_mode='Markdown'
        ),
        'settings': lambda: query.edit_message_text(
